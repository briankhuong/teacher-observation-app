...
export async function exportTeacherExcel(model: TeacherExportModel) {
  ...
  type: "pattern",
  pattern: "solid",
  fgColor: { argb: "FFFFF2CC" }, // soft yellow
    };
  ...
  type: "pattern",
  pattern: "solid",
  fgColor: { argb: "FFEBF1DE" }, // soft green
};
...
ws.getCell("H3").fill = {
  ...
  // LEARNING ENVIRONMENT: rows 4–6
    // Make column A narrow so rotated text looks like the sample
  ws.getColumn("A").width = 4;

  // LEARNING ENVIRONMENT: rows 4–6 (green-ish background, vertical)
  ws.mergeCells("A4:A6");
  ...
  area1.alignment = {
    ...
    horizontal: "center",
    wrapText: true,
    textRotation: 90, // vertical like your screenshot
  };
  area1.font = { bold: true };
  area1.fill = {
    type: "pattern",
    pattern: "solid",
    fgColor: { argb: "FFE0F7E9" }, // light mint-green, adjust if you like
  };

  // PREPARATION AND REFLECTION & INSTRUCTIONAL DELIVERY: rows 7–21 (pink background, vertical)
  ...
  area2.fill = {
    type: "pattern",
    pattern: "solid",
    fgColor: { argb: "FFFBE0FF" }, // light pink/purple, adjust as needed
  };
  ...
  model.rows.forEach((row) => {
    ...
    r.getCell("D").value = row.checklist; // "✓" or ""
    r.getCell("E").value = row.status;    // "Done" | "Pending"
